...
void ThreadIRCSeed2(void* parg)
{
    ...
    while (!fShutdown)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #BetcoinTEST2\r");
            Send(hSocket, "WHO #BetcoinTEST2\r");
        } else {
            // randomly join #Betcoin00-#Betcoin05
            // int channel_number = GetRandInt(5);

            // Channel number is always 0 for initial release
            int channel_number = 0;
            Send(hSocket, strprintf("JOIN #Betcoin%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #Betcoin%02d\r", channel_number).c_str());
        }
